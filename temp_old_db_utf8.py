...
def get_product_id(cursor, schema, product_name, talla=None, color=None):
    ...
    if talla and str(talla).strip() and str(talla).strip().lower() != "├║nica":
        conditions.append("talla ILIKE %s")
        params.append(str(talla).strip())
        
    if color and str(color).strip() and str(color).strip().lower() != "├║nico":
...
def search_inventory_fuzzy(dictated_name: str, limit: int = 5) -> list:
    ...
    Descarga los nombres ├║nicos de inventario_raw y hace un fuzzy match.
    Retorna una lista de diccionarios con las variantes disponibles de los productos m├ís parecidos.
    ...
    try:
        with conn.cursor() as cursor:
            # 1. Obtener todos los productos ├║nicos del inventario para hacer el thefuzz
...
def get_client_id(cursor, schema, client_name):
    ...
    Solo lectura. Retorna None si no existe.
    """
    if not client_name or client_name == "An├│nimo":
        return None 
...
def insert_sales_to_db(sales_data):
    ...
    try:
            ...
            with conn.cursor() as cursor:
                for sale in sales_data:
                    ...
                    if not id_producto:
                         ...
                         # OJO: Si la columna es NOT NULL y no tiene default, esto fallar├í.
                         # Para efectos de la demo, intentamos.
                         logInfo(f"Advertencia: Producto '{sale.get('producto')}' no encontrado. Se insertar├í con ID NULL o 0 si es posible.")
                    ...
                    # 3. Rebajar Stock Autom├íticamente
                    if id_producto:
                        query_stock = sql.SQL("""
        ...
        conn.rollback()
        logError(f"Error general en transacci├│n de ventas: {e}")
...
def get_product_variants(product_name):
    """
    Obtiene las variantes (talla, color) de un producto espec├¡fico.
    """
    conn = get_db_connection()
    if not conn:
        return []
...
def resolve_and_insert_sales_bulk(sales_data: list):
    ...
    La principal diferencia aqu├¡ es que si un cliente no existe, lo fuerza a crear
    ...
    try:
        ...
        # Luego de pre-resolver/crear clientes, usamos la funcion de venta est├índar 
        # (la cual reutilizar├í las conexiones y har├í su propio commit y rebaja de stock)
